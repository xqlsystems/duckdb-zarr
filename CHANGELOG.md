...
- **AnnData-style Zarr compatibility** (#40): a single array selected with `array_path=` that has no `dimension_names`/`_ARRAY_DIMENSIONS` — which AnnData/zarr-python never write — now gets `dim_0..dim_{ndim-1}` dimension names instead of failing bind, so AnnData's `obs`/`var` columns, sparse `X` components and `obsm` embeddings can be read directly. Whole-store enumeration is unchanged and still errors on such arrays (pointing at `array_path=`). `read_zarr_metadata` now lists an array with an unsupported dtype/codec as `role = 'unsupported'` instead of failing the whole call. No AnnData-aware parsing is added; see `docs/design.md` decision 7 for scope and for the null-handling and empty-row pitfalls of hand-written SQL, all covered by `test/sql/anndata.test`.
