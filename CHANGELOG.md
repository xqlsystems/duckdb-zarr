...
### Added
- **AnnData stores** (#40): `read_zarr(store, array_path := name)` now reads an array that declares no dimension names, as in every store written by AnnData or plain zarr-python. Its dimensions are named `dim_0`, `dim_1`, and so on. A whole-store `read_zarr(store)` still fails on such arrays, because placeholder names could join unrelated arrays; the error now suggests `array_path=`. See [Querying AnnData stores](docs/anndata.md) for SQL that reads `obs`/`var` columns, categoricals, and a sparse `X`.
- **`read_zarr_metadata` column `array_path_dims`**: the dimension names that `read_zarr(store, array_path := name)` binds for each array.
