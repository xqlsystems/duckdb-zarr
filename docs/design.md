...
### 7. AnnData-style stores: synthesize dim names, don't special-case the format

[Issue #40](https://github.com/xqlsystems/duckdb-zarr/issues/40) asks for compatibility with [AnnData's Zarr layout](https://github.com/scverse/anndata/blob/main/docs/fileformat-prose.md) — the on-disk convention used by the single-cell biology ecosystem (`scanpy`, `scverse`). AnnData writers (`anndata`/`zarr-python`) never write `dimension_names` or `_ARRAY_DIMENSIONS` on any array, so `dimension_names()` rejected every array in such a store at bind, even for a single-array `array_path=` read. A field test against a real pbmc3k AnnData store (issue #40, comment from `kentstephen`, against the shipped v0.1.3 extension) found this was in fact the *only* structural blocker: once an array opens, sparse CSR components (`X/data`/`X/indices`/`X/indptr`), categorical `codes`/`categories`, and `obs`/`var` columns all decode as ordinary scalar columns via `array_path=`, and CSR-to-long-form reconstruction, pseudobulk, and marker-gene queries all run as plain SQL (one ASOF join over `indptr`) with no densification.

> **Decision:** When an array has no `dimension_names` and no `_ARRAY_DIMENSIONS`, synthesize `dim_0..dim_{ndim-1}` from its rank instead of erroring at bind. Do **not** add AnnData-specific parsing (no automatic dataframe assembly, no categorical decoding, no CSR reconstruction) — this is a general "don't require metadata that a format doesn't produce" fix, not format-aware code. `array_path=` plus hand-written SQL remains the intended way to work with per-array component stores like AnnData's, consistent with the existing non-goal of not doing automatic relational joins across nested groups (§Non-goals).
>
> **Rationale:** The field test showed the two real blockers were missing dim metadata and missing string-dtype support (the latter fixed separately, see [PR #44](https://github.com/xqlsystems/duckdb-zarr/pull/44)) — everything else the AnnData/scverse ecosystem needs is already expressible as `array_path=` reads plus ordinary SQL, and the reporter explicitly called an encoding-aware convenience layer ("dataframe, categorical, csr_matrix" readers) "nice... eventually, but not a blocker." Building that layer now would be scope creep against evidence that it isn't needed to unblock real usage; it can land as a separate, later, stacked PR if demand shows up.
>
> **Consequence:** Whole-store enumeration (`infer_dim_groups`, used by `read_zarr(store)` and the replacement scan) also gets the fallback, so a non-xarray store's bare `read_zarr(store)` no longer aborts outright. But dims synthesized purely from rank give `infer_dim_groups` nothing meaningful to group same-shaped-but-unrelated arrays by — for an AnnData store this generally surfaces as the existing "multiple dimension groups" or shape/chunk-mismatch bind error, both of which already point the user at `array_path=`. That's accepted as the correct degradation, not a bug to fix here: it never produces silently-wrong output (the existing shape/chunk-uniformity check in decision 6 still catches accidental collisions), and `array_path=` is already the documented workaround for multi-group stores.
