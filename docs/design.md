...
### 7. AnnData-style stores: synthesize dim names for `array_path=`, don't special-case the format

[Issue #40](https://github.com/xqlsystems/duckdb-zarr/issues/40) asks for compatibility with [AnnData's Zarr layout](https://github.com/scverse/anndata/blob/main/docs/fileformat-prose.md) — the on-disk convention of the single-cell ecosystem (`scanpy`, `scverse`). AnnData writers never write `dimension_names` or `_ARRAY_DIMENSIONS`, so `dimension_names()` rejected every array in such a store at bind, even for a single-array `array_path=` read. A field test in the issue thread, against a real pbmc3k store on the v0.1.3 extension, found this and string dtype support (#44) to be the only structural blockers: once an array opens, sparse CSR components, categorical `codes`/`categories` and `obs`/`var` columns decode as ordinary columns via `array_path=`, and CSR-to-long-form, pseudobulk and marker-gene queries run as plain SQL with no densification.

> **Decision:** When a single array is selected with `array_path=` and has neither `dimension_names` nor `_ARRAY_DIMENSIONS` (nor OME `multiscales.axes`), name its dimensions `dim_0..dim_{ndim-1}`. Whole-store enumeration (`read_zarr(store)` without `array_path=`, the replacement scan, `read_zarr_groups`) is unchanged and still errors on such arrays, with a message pointing at `array_path=`. Do **not** add AnnData-specific parsing (no dataframe assembly, categorical decoding or CSR reconstruction): `array_path=` plus hand-written SQL is the intended interface, consistent with the non-goal of automatic relational joins across nested groups.
>
> **Why not synthesize during whole-store enumeration too:** positional names carry no meaning. If two unrelated same-length arrays both lacked metadata they would both be `dim_0`, be grouped, and be aligned row-for-row as if they shared a coordinate — a silent wrong answer where the previous behaviour was a loud error. `array_path=` is different because the user has named exactly one array, so there is nothing to mis-align. (`read_zarr_metadata.dims` keeps reporting only what the array's own metadata declares; the names `array_path=` will bind are reported separately in `array_path_dims`, so the columns to write in SQL are discoverable.)
>
> **Rationale for not building a convenience layer now:** the field test showed the real blockers were missing dim metadata and missing string dtype, and the reporter called encoding-aware readers "nice... eventually" rather than a blocker. It can land as a separate, later, stacked PR.

**What hand-written SQL has to get right.** With no AnnData-aware reader, these are the traps, all exercised in `test/sql/anndata.test`:

- **Nulls are stored out of band, and reading the data array alone loses them.** A string column with nulls is either a `nullable-string-array` *group* (`values` + boolean `mask`) — reading `values` returns `''` for a null, with no signal — or a categorical whose null cells have code `-1`, which an `INNER JOIN` to `categories` silently drops. Apply the `mask`; use a `LEFT JOIN` for categoricals. (This is separate from the CF-sentinel masking in §Type mapping, which is numeric-only and does not apply to strings.)
- **Empty rows break "last `indptr` offset at or before this position".** An empty row repeats an `indptr` offset, so a plain `ASOF JOIN` picks among ties and misassigns nonzeros. Two exact recipes: join on the half-open interval `[indptr[r], indptr[r+1])`; or `ASOF JOIN` against `indptr` deduplicated to the *last* row per offset (`SELECT value, MAX(dim_0) AS obs_idx FROM indptr GROUP BY value`). Both are tested and asserted identical.
- **Cost, and materialize first.** Measured on in-memory tables with 4 threads (engine-level join cost only — it excludes the `read_zarr` scan itself), reconstructing all nonzeros of a synthetic CSR with ~5% empty rows: 2.1M nnz (pbmc3k-sized) took 0.4–0.6 s / ~390 MB with the range join and 0.15 s / ~170 MB with the deduplicated ASOF; 9.5M nnz took 1.7 s / 1.6 GB versus 0.7 s / 0.8 GB. Both scale linearly (the range join runs as an inequality join, not a cross product), so "no densification" holds, but the reconstruction is not free — prefer the deduplicated ASOF at scale. Joining directly over several `read_zarr` calls has been reported to re-scan pathologically (~50 s versus ~0.25 s once materialized, in the issue #40 thread), so `CREATE TEMP TABLE` each component (`X/data`, `X/indices`, `X/indptr`) first and join those. That scan-side figure is a report from the thread, not something measured here.
- **`indptr` means different things for `csr_matrix` and `csc_matrix`.** For CSC it indexes columns and `indices` holds row ids. The extension does not read the group's `encoding-type`, so the caller must know which they have.

> **Also in this change:** `read_zarr_metadata` lists an array that zarrs cannot open because of an unsupported dtype, codec or metadata as `role = 'unsupported'` (`dtype = 'unsupported'`, the error text in `attrs`) instead of failing the whole call. Other failures — I/O, auth, missing metadata — still fail the call.
