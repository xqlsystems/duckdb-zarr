...
- **Empty rows break "last `indptr` offset at or before this position".** An empty row repeats an `indptr` offset, so an `ASOF JOIN` picks among ties and misassigns nonzeros. Join on the half-open interval `[indptr[r], indptr[r+1])` instead, which is exact.
